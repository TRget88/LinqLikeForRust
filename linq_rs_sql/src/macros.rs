...
//! `linq_rs_sql::table! { ... }` from anywhere.
...
/// ```
///
/// `SqlType` is one of the markers exported from [`crate::types`]:
...
///
/// - A `Marker` zero-sized type implementing [`Table`](crate::Table) —
///   the type-level identity of the table.
/// - One zero-sized type per column (lowercase, matching the column name)
///   implementing [`Column`](crate::Column) and
///   [`Expr`](crate::Expr). Because they're unit structs, the lowercase
///   name doubles as both a type and a value.
/// - A `table()` constructor returning a fresh
///   [`Query`](crate::Query) seeded with `SELECT *`.
///
/// # Example
///
/// ```rust
