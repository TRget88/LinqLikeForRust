...
Of the 74, **5** are static generator methods on `Enumerable` rather than extension methods. The 3 implemented here are free functions, not `LinqExt` methods: `empty`, `range`, `repeat`; `InfiniteSequence`, `Sequence` are not implemented.

> These numbers are generated by [`gen-docs.py`](https://github.com/TRget88/LinqLikeForRust/blob/main/.github/scripts/gen-docs.py) from [`.github/data/`](https://github.com/TRget88/LinqLikeForRust/tree/main/.github/data), and CI fails if they drift. Do not edit them by hand. (Neither path ships in the crate.)
...
a superseded 2026-09-09 snapshot kept for the record — not a description of the
crate today.
...
practice that means floats. `vec![1.0, 2.0, 1.0].into_iter().distinct()` does not
compile; `.into_iter().distinct_partial_eq()` does. The names are long deliberately: this is the O(n²) slow
