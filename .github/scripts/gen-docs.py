...
def render_coverage(mapping, csharp):
    ...
    lines = [
        ...
        "",
        # RR-014: these two paths are excluded from the published tarball, so a
        # reader on docs.rs or crates.io cannot follow them. Link to GitHub and
        # say so, rather than printing bare repo-relative paths.
        "> These numbers are generated by"
        " [`gen-docs.py`](https://github.com/TRget88/LinqLikeForRust/blob/main/.github/scripts/gen-docs.py)"
        " from"
        " [`.github/data/`](https://github.com/TRget88/LinqLikeForRust/tree/main/.github/data),"
        " and CI fails if they drift. Do not edit them by hand."
        " (Neither path ships in the crate.)",
    ]
    return "\n".join(lines)
